...
interface SaleInvoicePrintModalProps {
  isOpen: boolean;
  onClose: () => void;
  invoice: any;
}
 ...
 * SaleInvoicePrintModal. That meant a brand-new function (== a brand
 * new component type, to React) was created on every render. Every
...
const InvoiceDocument = ({
  ...
  >
    ...
    <div className="space-y-2 mb-4">
      <FieldLine label="Date" value={getEdited("saleDate")} onChange={(v) => handleChange("saleDate", v)} />
      ...
      <div className="grid grid-cols-2 gap-x-6">
        <FieldLine
          label="On this day"
          value={getEdited("saleDate2", invoice.saleDate || "")}
          onChange={(v) => handleChange("saleDate2", v)}
        />
        <FieldLine label="Time" value={getEdited("saleTime")} onChange={(v) => handleChange("saleTime", v)} />
      </div>

      <FieldLine
        label="Registration Named"
        value={getEdited("registrationNamed", invoice.buyerName || "")}
        onChange={(v) => handleChange("registrationNamed", v)}
      />
      <FieldLine
        label="Total Amount"
        value={getEdited(
          "totalAmount",
          salePriceVal ? `Rs. ${Number(salePriceVal).toLocaleString()}` : ""
        )}
    ...
    <div className="grid grid-cols-2 gap-x-8">
      ...
      <FieldLine label="Name:" value={getEdited("ownerName")} onChange={(v) => handleChange("ownerName", v)} />
      <FieldLine label="Name:" value={getEdited("buyerName")} onChange={(v) => handleChange("buyerName", v)} />

      <FieldLine
        label="S/o"
        value={getEdited("ownerFatherName")}
        onChange={(v) => handleChange("ownerFatherName", v)}
      />
      <FieldLine
        label="S/o"
        value={getEdited("buyerFatherName")}
        onChange={(v) => handleChange("buyerFatherName", v)}
      />

      <FieldLine label="RES:" value={getEdited("sellerRes")} onChange={(v) => handleChange("sellerRes", v)} />
      <FieldLine label="RES:" value={getEdited("buyerRes")} onChange={(v) => handleChange("buyerRes", v)} />

      <FieldLine
        label="Phone No:"
        value={getEdited("ownerPhone")}
        onChange={(v) => handleChange("ownerPhone", v)}
      />
      <FieldLine
        label="Phone No:"
        value={getEdited("buyerPhone")}
        onChange={(v) => handleChange("buyerPhone", v)}
      />
      ...
      <div className="mt-3">
        <FieldLine
          label="Signature"
          value={getEdited("buyerSignature")}
          onChange={(v) => handleChange("buyerSignature", v)}
        />
      </div>
      ...
      <FieldLine
        label="Witness"
        value={getEdited("buyerWitness")}
        onChange={(v) => handleChange("buyerWitness", v)}
      />
    </div>
...
export const SaleInvoicePrintModal = ({ isOpen, onClose, invoice }: SaleInvoicePrintModalProps) => {
  ...
  const salePriceVal = parseFloat(invoice.salePrice || "0");
  const downPaymentVal = parseFloat(invoice.downPayment || invoice.tokenAmount || "0");
  const remainingAmountVal = salePriceVal - downPaymentVal;
        ...
        <div className="w-full max-w-4xl h-[90vh] bg-white overflow-hidden rounded-lg flex flex-col">
          <div className="flex items-center justify-between p-4 border-b border-gray-300 bg-white shrink-0">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-semibold text-gray-900">Sale Invoice Print Preview</h3>
            </div>

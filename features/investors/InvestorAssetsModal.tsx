...
interface Investor extends DocumentData {
    id: string;
    name: string;
    phone: string;
...
interface InvestorAssetsModalProps {
    isOpen: boolean;
    onClose: () => void;
...
export const InvestorAssetsModal = ({ isOpen, onClose, investor }: InvestorAssetsModalProps) => {
    ...
    useEffect(() => {
        if (investor?.assets) {
            setLocalAssets(investor.assets);
        } else {
            setLocalAssets([]);
        }
    }, [investor]);
        ...
        try {
            ...
            reader.onloadend = () => {
                ...
                img.onload = () => {
                    ...
                    pdf.save(`${investor.name.replace(/\s+/g, '_')}_Asset_${index + 1}.pdf`);
            ...
            <Card className="w-full max-w-3xl max-h-[85vh] flex flex-col shadow-xl bg-card overflow-hidden">
                ...
                <div className="flex items-center justify-between p-4 border-b border-border bg-muted">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="font-semibold text-foreground">Investor Assets</h3>
                            <p className="text-xs text-muted-foreground">{investor.name} • {investor.phone}</p>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-6 bg-muted/50">
                        {localAssets.length > 0 ? (
                            ...
                            <div className="h-full flex flex-col items-center justify-center text-muted-foreground space-y-3 min-h-[300px]">
                                ...
                                <p className="text-xs max-w-[200px] text-center">Upload photos of ID cards, documents, or other investor related files.</p>
                            </div>
                        )}
                    </div>

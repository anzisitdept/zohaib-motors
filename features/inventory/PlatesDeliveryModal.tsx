...
import { X, Printer, Car, Calendar, Zap, Fuel, Settings2, Package, User, Hash, MapPin, Phone, CreditCard, PenTool } from "lucide-react";
...
interface PlatesDeliveryModalProps {
    isOpen: boolean;
    onClose: () => void;
    vehicle: any;
}

export const PlatesDeliveryModal = ({ isOpen, onClose, vehicle }: PlatesDeliveryModalProps) => {
    ...
    useEffect(() => {
        ...
        const fetchClients = async () => {
            ...
            // Fetch Plates Delivery Recipient
            if (vehicle.plateDeliveredToClientId) {
                try {
                    const snapDr = await getDoc(doc(db, "clients", vehicle.plateDeliveredToClientId));
    ...
    const DetailRow = ({ label, value, icon: Icon }: { label: string, value: string | number | undefined, icon?: any }) => (
        <div className="flex items-center justify-between py-2 border-b border-border last:border-0">
            ...
            <span className="text-sm font-semibold text-foreground text-right">{value || "Not Set"}</span>
        </div>
    );
        ...
        <div className="bg-card p-8 md:p-12 max-w-4xl mx-auto text-foreground font-sans relative">
            ...
            <div className="flex justify-between items-start border-b-2 border-slate-900 pb-6 mb-8">
                <div className="flex flex-col">
                    ...
                    <h1 className="text-3xl font-bold uppercase tracking-tight text-foreground">Plates Delivery Note</h1>
                    <p className="text-sm text-muted-foreground font-medium mt-1">Official Number Plates Handover</p>
                </div>
                <div className="text-right space-y-1">
                    <div className="bg-purple-700 text-white px-4 py-1.5 text-sm font-bold tracking-widest inline-block mb-2">
                        DELIVERED
                    </div>
                    <p className="text-xs text-muted-foreground uppercase font-bold">Delivered By</p>
                    <p className="text-sm font-semibold">{userData?.name || user?.displayName || "Staff"}</p>
            ...
            <div className="grid grid-cols-2 gap-12 mb-8">
                ...
                <div>
                    ...
                    <div className="space-y-1">
                        ...
                        <DetailRow label="Vehicle CC" value={vehicle.vehicleCC} icon={Zap} />
                        <DetailRow label="Source" value={vehicle.vehicleSource} icon={Fuel} />
                    </div>
                </div>

                {/* --- Recipient Details (Who plates were delivered to) --- */}
                <div>
                    <h3 className="text-sm font-bold uppercase text-foreground mb-4 border-b border-border pb-2 flex items-center gap-2">
                        <User size={16} /> Number Plates Delivered To
                    </h3>
                    <div className="space-y-1">
                        <DetailRow label="Recipient Name" value={deliveryRecipient?.name || vehicle.plateDeliveredToName || vehicle.ownerName} />
                        <DetailRow label="Phone" value={deliveryRecipient?.phone || vehicle.plateDeliveredToPhone || vehicle.ownerContact} icon={Phone} />
                        ...
                        {vehicle.plateDeliveryNotes && <DetailRow label="Notes" value={vehicle.plateDeliveryNotes} />}
                    </div>
                </div>
            </div>

            {/* --- Delivery Note Section --- */}
            {vehicle.plateDeliveryNote && (
                <div className="mb-8 p-4 bg-muted rounded-lg border border-purple-200">
                    <h3 className="text-sm font-bold uppercase text-foreground mb-3 flex items-center gap-2">
                        <Package size={16} className="text-primary" /> Delivery Note
                    </h3>
            ...
            <div className="mt-16 mb-8">
                ...
                <p className="text-xs text-muted-foreground mb-8 italic">
                    By signing below, I acknowledge that I have received the vehicle number plates described above in good condition and as agreed.
                </p>

                <div className="grid grid-cols-2 gap-12 mt-12">
                    <div>
                        {deliveryRecipient?.signatureUrl && (
                            <div className="mb-4">
                                {/* eslint-disable-next-line @next/next/no-img-element */}
                                <img
                                    src={deliveryRecipient.signatureUrl}
                ...
                <Card className="w-full max-w-4xl max-h-[90vh] flex flex-col shadow-xl bg-card overflow-hidden">
                    <div className="flex items-center justify-between p-4 border-b border-border bg-muted">
                        <div className="flex items-center gap-2">
                            ...
                            <h3 className="font-semibold text-foreground">Print Preview - Plates Delivery Note</h3>
                        </div>
